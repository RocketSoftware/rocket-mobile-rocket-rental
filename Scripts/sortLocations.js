(function() {
	...
	function callback() {
		var sortfunction;
		var label;
		if (RocketMobileApplication.getEntity("distanceInKM").getValue() == true) {
			sortfunction = getDistanceFromLatLonInKm;
			label = " km";
		} else {
			sortfunction = getDistanceFromLatLonInMiles;
			label = " mi";
		}
		...
		for (var i = 0; i < officeLocations.length; i++) {
			...
			officeLocations[i].distance = round(sortfunction(
					myLat, myLon, lat, lon));
			officeLocations[i].display = officeLocations[i].Name + " ("
					+ officeLocations[i].distance + label + ")";
		}
